...
// ============================================================
// CONDIVIDI QUANTOTENGO — privacy-friendly, senza widget social
// ============================================================
function getQuantoTengoShareData(context) {
  if (context === 'vinted') {
    return {
      title: 'Calcolatore Guadagno Vinted | QuantoTengo',
      text: 'Scopri quanto ti resta davvero da una vendita su Vinted: prezzo, costi, spedizione, Bump e offerte.',
      url: 'https://www.quantotengo.it/vinted/'
    };
  }
  return {
    title: 'QuantoTengo — calcolatori guadagno netto per vendite online',
    text: 'Scopri quanto ti resta davvero da una vendita online con QuantoTengo.',
    url: 'https://www.quantotengo.it/'
  };
}

function setShareButtonFeedback(btn, text) {
  if (!btn) return;
...
function copiaLinkQuantoTengo(context, btn) {
  const data = getQuantoTengoShareData(context);
  const done = () => {
    setShareButtonFeedback(btn, '✓ Link copiato');
    trackEvent('share_copy_link_done', { context: context || CFG.id || 'calcolatore' });
  };
  const fallback = () => {
    const ta = document.createElement('textarea');
    ta.value = data.url;
    ...
    catch(e) { prompt('Copia questo link:', data.url); }
    document.body.removeChild(ta);
  };

  if (navigator.clipboard && navigator.clipboard.writeText) {
    navigator.clipboard.writeText(data.url).then(done).catch(fallback);
  } else {
    fallback();
  }
}

function condividiQuantoTengo(context, btn) {
  const data = getQuantoTengoShareData(context);
  trackEvent('share_native_opened', { context: context || CFG.id || 'calcolatore' });

  if (navigator.share) {
    navigator.share(data)
      .then(() => trackEvent('share_native_completed', { context: context || CFG.id || 'calcolatore' }))
      .catch(err => {
        if (!err || err.name !== 'AbortError') copiaLinkQuantoTengo(context, btn);
      });
  } else {
    copiaLinkQuantoTengo(context, btn);
  }
}
