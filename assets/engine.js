...
// STATE
// ============================================================
let spedOn = false, bumpOn = false, offerteOpen = true;
...
function esportaCSV() {
  ...
  const headers = ['Data','Articolo','Prezzo (€)','Costo Acquisto (€)','Spedizione (€)','Bump (€)','Commissione Vinted (€)','Guadagno Netto Stimato (€)'];
  const rows = storicoList.map(item => [
    '"' + item.data + '"',
    '"' + (item.nome || 'Articolo senza nome') + '"',
    item.prezzo.toFixed(2).replace('.',','),
    item.costo.toFixed(2).replace('.',','),
    item.sped != null ? item.sped.toFixed(2).replace('.',',') : '0,00',
    item.bump != null ? item.bump.toFixed(2).replace('.',',') : '0,00',
    item.comm.toFixed(2).replace('.',','),
    item.netto.toFixed(2).replace('.',',')
  ]);
  ...
  a.href = url;
  a.download = 'storico-vinted-' + dateStr + '.csv';
